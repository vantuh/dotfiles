...
export async function discoverKiroModels(cwd = process.cwd()): Promise<KiroModelConfig[]> {
  ...
  try {
    ...
    const session = await client.request("session/new", { cwd, mcpServers: [] }, 30000) as any;
    const available = session?.models?.availableModels;
    if (!Array.isArray(available) || available.length === 0) return KIRO_MODELS;

    return dedupeModels(available.map(normalizeDiscoveredModel).filter(Boolean) as KiroModelConfig[]);
  } finally {
    await client.stop();
  }
}
