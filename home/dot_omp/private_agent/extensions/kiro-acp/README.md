...
the host is `omp`. Chezmoi maps it to `~/.omp/agent/extensions/kiro-acp`.
...
| Config | `~/.omp/agent/kiro-acp.json` → [`home/dot_omp/private_agent/.kiro-acp.json`](../../.kiro-acp.json) (symlink) | `~/.pi/agent/kiro-acp.json` |
| Host binary | `omp` | `pi` |
