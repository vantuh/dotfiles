...
function escapeFrameDelimiters(text: string): string {
	return text.split(KIRO_TOOL_FRAME_PREFIX).join(ESCAPED_PREFIX).split(KIRO_TOOL_FRAME_SUFFIX).join(ESCAPED_SUFFIX);
}
...
export function stripAssistantContentFrames(content: unknown): { content: unknown[]; changed: boolean } {
