...
function fakeSession(modelId: string): { session: AcpSession; written: string[] } {
...
async function main(): Promise<void> {
	...
	try {
		// --- a prompt error with no consumer must not become an unhandled rejection ---
		{
			...
			assert(frame.method === "session/prompt", "startPrompt sends session/prompt");
			assert(session.activePromptDone !== null, "startPrompt records the in-flight prompt");

			// Nobody attaches handlers (the stream never got that far).
			session.handleStdoutLine(
				JSON.stringify({ jsonrpc: "2.0", id: frame.id, error: { code: -32603, message: "Internal error" } }),
			);
			await tick();

			assert(unhandled.length === 0, "a prompt error with no consumer raises no unhandled rejection");
			assert(
		...
		// --- consumers still observe the failure ---
		{
			...
			session.handleStdoutLine(
				JSON.stringify({ jsonrpc: "2.0", id: frame.id, error: { code: -32603, message: "boom" } }),
			);
			assert((await settled) === "boom", "the safety net does not swallow the error from real consumers");
		}
		...
		// --- a successful prompt still resolves and clears the recorded error ---
		{
			...
			await session.startPrompt("m1", "", "hi");
			const frame = JSON.parse(written[0]);
			assert(session.lastPromptError === null, "startPrompt clears the previous turn's error");
			...
			assert((await settled) === "end_turn", "a successful prompt resolves with its stopReason");
		}
		...
		await tick();
		assert(unhandled.length === 0, "no unhandled rejections across the whole test");
