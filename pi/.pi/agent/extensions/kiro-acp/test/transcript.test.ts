...
// --- transcript building ---
{
	assert(buildConversationPrompt(ctx([])) === "", "no user message yields an empty prompt");
	assert(buildConversationPrompt(ctx([assistantText("orphan")])) === "", "assistant-only history yields an empty prompt");

	const single = buildConversationPrompt(ctx([user("hello")]));
	assert(single === "<current_user_message>\nhello\n</current_user_message>", "a lone user message needs no history block");

	const withHistory = buildConversationPrompt(ctx([
		user("first"),
		assistantText("answer"),
		user("second"),
	]));
	assert(withHistory.includes("<conversation_history>"), "prior turns are wrapped in conversation_history");
	assert(withHistory.includes('<message role="user">\nfirst\n</message>'), "history keeps earlier user turns");
	assert(withHistory.includes("<text>\nanswer\n</text>"), "history keeps assistant text");
	assert(withHistory.endsWith("<current_user_message>\nsecond\n</current_user_message>"), "the latest user message ends the prompt");
	assert(!withHistory.includes("<message role=\"user\">\nsecond"), "the current message is not duplicated into history");

	const escaped = buildConversationPrompt(ctx([user("a & b <tag> c")]));
	assert(escaped.includes("a &amp; b &lt;tag&gt; c"), "user text is XML-escaped");

	const toolCallHistory = buildConversationPrompt(ctx([
		user("q"),
		{ role: "assistant", content: [{ type: "toolCall", id: 'c"1', name: "web_search", arguments: { query: "x" } }] },
		{ role: "toolResult", toolCallId: 'c"1', toolName: "web_search", isError: true, content: [{ type: "text", text: "failed <hard>" }] },
		user("q2"),
	]));
	assert(toolCallHistory.includes('<tool_call id="c&quot;1" name="web_search">'), "tool call attributes are escaped");
	assert(toolCallHistory.includes('is_error="true"'), "tool result errors are marked in history");
	assert(toolCallHistory.includes("failed &lt;hard&gt;"), "tool result text is escaped");

	const long = "x".repeat(13000);
	const truncated = buildConversationPrompt(ctx([user(long), assistantText("ok"), user("now")]));
	assert(truncated.includes("[...truncated 1000 chars...]"), "long history messages are truncated");
	const currentLong = buildConversationPrompt(ctx([user(long)]));
	assert(!currentLong.includes("truncated"), "the current user message is never truncated");

	assert(lastUserMessage(ctx([user("a"), assistantText("b"), user("c")])) === "c", "lastUserMessage returns the latest user turn");
	assert(lastUserMessage(ctx([
...
{
	const results = extractToolResults(ctx([
		user("q"),
		assistantText("thinking"),
		{ role: "toolResult", toolCallId: "id-1", toolName: "web_search", isError: false, content: [{ type: "text", text: "one" }] },
		{ role: "toolResult", toolCallId: "id-2", toolName: "fetch_content", isError: true, content: [{ type: "text", text: "two" }] },
	]));
	assert(results.length === 2, "trailing tool results are collected");
	assert(results[0].toolCallId === "id-1" && results[1].toolCallId === "id-2", "tool results keep transcript order");
	assert(results[1].isError === true, "error flags survive extraction");
	assert(results[0].content === undefined, "text-only results carry no content blocks");

	const withImage = extractToolResults(ctx([
		{ role: "toolResult", toolCallId: "id-3", toolName: "shell", isError: false, content: [{ type: "text", text: "shot" }, { type: "image", data: "AAA", mimeType: "image/png" }] },
	]));
	assert(withImage[0].content?.length === 2, "image results keep their content blocks");
	assert(withImage[0].text === "shot", "image results still expose their text");

	const stopsAtAssistant = extractToolResults(ctx([
		{ role: "toolResult", toolCallId: "old", toolName: "web_search", isError: false, content: [{ type: "text", text: "old" }] },
		assistantText("newer turn"),
	]));
	assert(stopsAtAssistant.length === 0, "extraction stops at the last assistant message");
}
...
// --- history fingerprints ---
{
	...
	assert(fp === historyFingerprintBeforeCurrentUser(ctx([...base])), "fingerprints are stable for identical history");
	assert(fp !== historyFingerprintBeforeCurrentUser(ctx([user("first"), assistantText("other"), user("current")])), "changed history changes the fingerprint");
	assert(fp === historyFingerprintBeforeCurrentUser(ctx([user("first"), assistantText("answer"), user("different current")])), "the current user message is excluded");
	...
	assert(
		historyFingerprintBeforeCurrentUser(argsOrderA) === historyFingerprintBeforeCurrentUser(argsOrderB),
		"tool-call argument key order does not change the fingerprint",
	);

	const assistant: AssistantMessage = { role: "assistant", content: [{ type: "text", text: "reply" }] } as any;
	const after = historyFingerprintAfterAssistantTurn(ctx(base), assistant);
	assert(after !== fp, "the post-turn fingerprint differs from the pre-turn one");
	assert(
		after === historyFingerprintBeforeCurrentUser(ctx([...base, assistant, user("next")])),
...
// --- persisted session store ---
{
	...
	assert(key !== persistenceKeyForSession("pi-session-1", "/tmp/other"), "the same pi session in another cwd gets another key");
	assert(persistenceKeyForSession("a/b:c", "/tmp/project").endsWith("a_b_c"), "unsafe id characters are sanitized");

	assert(loadPersistedKiroSession(key) === null, "loading an unknown key yields null");
	...
	assert(loaded?.historyFingerprint === "fp-1", "the history fingerprint round-trips");
	...
	assert(loadPersistedKiroSession(key) === null, "sessions older than the TTL are ignored");

	savePersistedKiroSession(key, { version: 2, kiroSessionId: "acp-future", historyFingerprint: "fp" } as any);
	assert(loadPersistedKiroSession(key) === null, "unknown persistence versions are ignored");
	...
	assert(loadPersistedKiroSession(key)?.kiroSessionId === "acp-99", "saving overwrites the previous record");
