...
const toolResultContext = (): Context => ({
	messages: [
		...
		{ role: "assistant", content: [{ type: "toolCall", id: "c1", name: "herdr_agent", arguments: {} }] },
...
async function main(): Promise<void> {
	// --- an orphaned tool result reuses the live session instead of forking ---
	{
		...
		assert(first.orphanedToolResults === false, "a plain turn is not flagged as orphaned");

		// Kiro dropped its tools/call, so nothing is pending when the result lands.
		assert(first.session.pendingToolCalls.size === 0, "no pending bridge call remains after the drop");

		const recovered = await routeSession(toolResultContext(), opts("S1"));
		assert(recovered.session === first.session, "the orphaned result is routed back to the live session");
		assert(recovered.isResumption === false, "there is no MCP request left to resume");
		assert(recovered.orphanedToolResults === true, "the orphaned result is flagged for recovery");
		assert(recovered.toolResults.length === 1, "the tool result is carried through");
	...
	// --- a matching pending call still takes the normal resumption path ---
	{
		...
		assert(resumed.session === routed.session, "resumption stays on the same session");
		assert(resumed.isResumption === true, "a matching pending call resumes normally");
		assert(resumed.orphanedToolResults === false, "resumption is not treated as orphaned");
		await stopAllSessions();
	}
	...
	// --- no live session to recover into: still flagged, so the replay is forced ---
	{
		...
		assert(routed.orphanedToolResults === true, "a cold start with tool results is flagged as orphaned");
		assert(routed.session.acpSessionId === null, "the cold session has no ACP conversation yet");
		await stopAllSessions();
	}
	...
	// subagent launches.
	{
		...
		assert(session.pendingToolCalls.size === 0, "the abandoned call stops being pending");
		...
		assert(dispatched.length === 1, "the retry is not dispatched to pi a second time");
		...
		assert(dispatched.length === 2, "a genuinely different call is still dispatched");
		different.abort.abort();
		await different.result;

		session.clearAbandonedToolCalls([{ toolCallId: "x", toolName: "herdr_agent", text: "done", isError: false }]);
		const afterRecovery = bridgeCall({ agent: "reviewer", task: "compare" });
		assert(dispatched.length === 3, "after recovery the same call may run again");
		...
		assert(dispatched.length === 5, "a call aborted while stopped leaves no dedup record");
	...
	// --- a late-settling prompt releases the session ---
	{
		...
		session.handleStdoutLine(JSON.stringify({ jsonrpc: "2.0", id: frame.id, result: { stopReason: "end_turn" } }));
		await tick();
		assert(session.activePromptDone === null, "a settled prompt is no longer recorded as active");
		assert(session.busy === false, "the session becomes reusable instead of busy forever");
	}
	...
	// --- a settled prompt does not clear a newer one ---
	{
		...
		session.handleStdoutLine(JSON.stringify({ jsonrpc: "2.0", id: firstFrame.id, result: { stopReason: "end_turn" } }));
		await tick();
		assert(session.activePromptDone === secondPrompt, "an older prompt settling leaves the newer one in place");
	}
	...
	// in 1ms, then Kiro relaunched the subagent.
	{
		...
		await tick();
		assert(session.busy === true, "the session stays busy while the old prompt is cancelled");
		const methods = written.map((line) => JSON.parse(line).method);
		assert(methods.includes("session/cancel"), "an in-flight prompt is cancelled before recovery");

		session.handleStdoutLine(JSON.stringify({ jsonrpc: "2.0", id: firstFrame.id, result: { stopReason: "cancelled" } }));
		await followUp;

		const frames = written.map((line) => JSON.parse(line));
		const prompts = frames.filter((frame) => frame.method === "session/prompt");
		assert(prompts.length === 2, "recovery sends a new prompt after cancel, not on top of it");
		assert(
			String(prompts[1].params.prompt[0].text).includes("dropped_tool_result"),
			"the follow-up prompt carries the dropped tool result",
		);
		assert(session.busy === true, "the recovery prompt is now the in-flight one");
	}
...
main().catch((error) => {
	console.error(`✗ orphaned-tool-result test failed: ${error instanceof Error ? error.stack || error.message : String(error)}`);
	process.exitCode = 1;
