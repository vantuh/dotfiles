...
const start = (id: string, title: string, meta?: unknown) =>
	({ sessionUpdate: "tool_call", toolCallId: id, title, _meta: meta });
const chunk = (id: string, text: string) =>
	({ sessionUpdate: "tool_call_update", toolCallId: id, content: [{ content: { text } }] });
...
// A finished tool: working message on start, one framed block on completion.
{
	...
	assert(calls.length === 1 && calls[0] === "work:🔧 Reading app.ts", "start only sets the working message");

	mirror.update(chunk("t1", "line one\n"));
	mirror.update(chunk("t1", "line two"));
	assert(calls.length === 1, "body chunks emit nothing until the tool finishes");
...
{
	const { calls, mirror } = harness();
	mirror.update(start("t1", "pi_host tool", { kiro: { mcpServerName: "pi_host" } }));
...
// Two overlapping tools must produce two separate, non-nested blocks.
{
	...
	assert(pushes[0].includes("tool A") && pushes[0].includes("body A"), "block A carries only A's body");
	assert(pushes[1].includes("tool B") && pushes[1].includes("body B"), "block B carries only B's body");
	assert(
	...
	assert(clearedAt > calls.lastIndexOf(pushes[1]) - 2, "working message survives while a tool is still running");
}
...
// An interrupted turn must not silently drop in-flight tools.
{
	...
	assert(pushes.every((p) => p.includes("[aborted]")), "in-flight tools are marked aborted");
	assert(pushes[0].includes("partial output"), "partial output collected so far is kept");
	assert(calls[calls.length - 1] === "work:(cleared)", "flush clears the working message");

	const before = calls.length;
	mirror.flush();
	assert(calls.length === before + 1, "a second flush emits no blocks, only the working-message clear");
}

// Malformed / unknown updates must not throw or leak.
{
	const { calls, mirror } = harness();
	mirror.update({ sessionUpdate: "tool_call_update", toolCallId: "ghost", status: "completed" });
	mirror.update({ sessionUpdate: "tool_call" });
	mirror.update(start("t1", "titleless tool"));
	mirror.update({ sessionUpdate: "tool_call_update", toolCallId: "t1", content: "not an array" });
...
{
	const { calls, mirror } = harness();
	mirror.update({ sessionUpdate: "tool_call", toolCallId: "t1", _meta: { kiro: { toolName: "grepSearch" } } });
	mirror.update(finish("t1", "completed"));
	assert(calls[0] === "work:🔧 grepSearch", "toolName is used as the title fallback");
}
