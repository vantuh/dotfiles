...
import { activeSessionCount, pruneIdleSessions, routeSession, stopAllSessions } from "../session-manager.ts";
...
async function waitForPortClosed(port: number, timeoutMs = 3000): Promise<boolean> {
...
function pendingCall(session: AcpSession, seq: number, toolName: string): {
	callId: string;
	...
	const settled = new Promise<{ result: string; isError?: boolean }>((r) => { resolveFn = r; });
...
const toolResult = (toolCallId: string, toolName: string, text = "ok"): ToolResultInfo => ({
	toolCallId,
	toolName,
	text,
	isError: false,
...
async function main(): Promise<void> {
	...
	assert(bridgeA.port !== bridgeB.port, "concurrent sessions listen on different ports");
	assert(bridgeA.token !== bridgeB.token, "concurrent sessions use different bearer tokens");
	...
	assert(callA.callId.startsWith(`${a.id}-`), "pending tool call ids carry the session prefix");

	// A result minted by another session must not be matched by name.
	a.deliverToolResults([toolResult(`${b.id}-1`, "web_search")]);
	assert(a.pendingToolCalls.size === 1, "a foreign-prefixed tool result cannot resolve another session's call");
	...
	assert(deliveredA.result === "delivered", "own-prefixed id resolves via unique tool-name match");
	...
	assert(b.matchingToolResults([toolResult(callB.callId, "web_search")]).length === 1, "own tool result matches its session");
	assert(b.matchingToolResults([toolResult(`${a.id}-1`, "web_search")]).length === 0, "foreign tool result matches no session");
	...
	assert(/Shutting down/.test(settledB.result), "stopped calls report shutdown");
	assert(/Stopped/.test((await rejected).message), "in-flight RPCs are rejected on stop");
	...
	assert(await isListening(bridgeA.port), "stopping one session leaves the other's bridge alive");

	await a.stop();
	assert(await waitForPortClosed(bridgeA.port), "stop closes session A's port");

	// --- idle prune closes bridges of sessions it drops ---
	const routed = await routeSession({ messages: [] } as any, { cwd: "/tmp" } as any);
	...
	assert(await waitForPortClosed(bridgeC.port), "pruned idle sessions release their port");

	// --- stopAllSessions leaves nothing behind ---
	const routed2 = await routeSession({ messages: [] } as any, { cwd: "/tmp" } as any);
	const bridgeD = await attachBridge(routed2.session);
	await stopAllSessions();
	assert(activeSessionCount() === 0, "stopAllSessions empties the registry");
	assert(await waitForPortClosed(bridgeD.port), "stopAllSessions releases every port");
