...
import { mkdirSync, writeFileSync, unlinkSync, rmSync, renameSync } from "node:fs";
...
import type { PendingRpc, PendingToolCall, SessionUpdate, ToolResultInfo } from "./types.ts";
...
export class AcpSession {
  ...
  rpcSend(method: string, params: unknown, timeoutMs = 60000): Promise<unknown> {
  ...
  handleStdoutLine(line: string): void {
    ...
    if (hasId && !hasMethod) {
      ...
      if (msg.method === "session/request_permission") {
        ...
        this.rpcRespond(msg.id, { outcome: { outcome: "selected", optionId: optId } });
      } else {
  ...
  async ensureStarted(tools: Context["tools"]): Promise<void> {
    ...
    log("starting kiro session", { session: this.id, cwd: this.cwd, agentRootPath: this.agentRootPath, agentName: this.agentName });
    this.proc = spawn("kiro-cli", ["acp", "--agent", this.agentName, "--trust-all-tools"], {
    ...
    this.proc.stderr?.on("data", (chunk) => log("kiro stderr", { session: this.id, text: String(chunk).slice(0, 1000) }));
  ...
  async startPrompt(modelId: string, systemPrompt: string, userMessage: string): Promise<void> {
    if (!this.acpSessionId) {
      const result = await this.rpcSend("session/new", { cwd: this.cwd, mcpServers: [] }) as any;
      this.acpSessionId = result.sessionId;
      log("acp session/new", { session: this.id, acpSessionId: this.acpSessionId });
    }

    if (this.currentModelId !== modelId) {
      await this.rpcSend("session/set_model", { sessionId: this.acpSessionId, modelId }, 30000);
    ...
    this.activePromptDone = (this.rpcSend("session/prompt", {
  ...
  private cleanupAfterProcessExit(): void {
    ...
    for (const [, call] of this.pendingToolCalls) call.resolve({ result: "kiro-cli exited", isError: true });
    ...
    this.ipcServer = null;
    this.ipcPort = null;
    if (this.toolsFilePath) { try { unlinkSync(this.toolsFilePath); } catch {} this.toolsFilePath = null; }
    if (this.agentConfigPath) { try { unlinkSync(this.agentConfigPath); } catch {} this.agentConfigPath = null; }
    if (this.agentRootPath) { try { rmSync(this.agentRootPath, { recursive: true, force: true }); } catch {} this.agentRootPath = null; }
  }
  ...
  async stop(): Promise<void> {
    ...
    for (const [, call] of this.pendingToolCalls) call.resolve({ result: "Shutting down", isError: true });
    this.pendingToolCalls.clear();
  ...
  deliverToolResults(toolResults: ToolResultInfo[]): void {
    for (const tr of toolResults) {
      ...
      if (match) {
        ...
        log("delivering tool result", { session: this.id, callId, toolName: call.toolName, resultLen: tr.text.length });
        call.resolve({ result: tr.text, isError: tr.isError });
      } else {
        log("UNMATCHED tool result", { session: this.id, toolCallId: tr.toolCallId, toolName: tr.toolName, pendingCalls: [...this.pendingToolCalls.keys()] });
      }
    }
  }

  private findToolCallMatch(tr: ToolResultInfo): [string, PendingToolCall] | null {
    ...
    if (!tr.toolCallId.startsWith(this.id + "-")) {
      log("findToolCallMatch: rejecting name-match (foreign toolCallId)", { session: this.id, toolCallId: tr.toolCallId, toolName: tr.toolName });
      return null;
    }
    const nameMatches = [...this.pendingToolCalls.entries()].filter(([, call]) => call.toolName === tr.toolName);
    return nameMatches.length === 1 ? nameMatches[0] : null;
  }

  private async startIpcServer(): Promise<void> {
    this.ipcServer = createServer(async (req, res) => this.handleIpcRequest(req, res));
  ...
  private async handleIpcRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
    try {
      ...
      if (req.method === "POST" && req.url === "/tool/pending") {
        ...
        const resultPromise = new Promise<{ result: string; isError?: boolean }>((resolve) => {
          const call: PendingToolCall = { callId: publicCallId, rawCallId, toolName, args, resolve };
          this.pendingToolCalls.set(publicCallId, call);
          log("IPC tool call received", { session: this.id, callId: publicCallId, rawCallId, toolName, argsKeys: Object.keys(args) });
      ...
      log("IPC error", { session: this.id, error: error instanceof Error ? error.message : String(error) });
      if (!res.headersSent) httpRespond(res, 500, { error: "Internal error" });
    }
  }

  private static readonly EXCLUDED_TOOLS = new Set(["Agent", "get_subagent_result", "steer_subagent"]);
  ...
  private writeTools(tools: Context["tools"]): void {
    ...
    const mcpTools = (tools || []).filter((t) => !AcpSession.EXCLUDED_TOOLS.has(t.name)).map((t) => ({
      name: t.name,
    ...
    writeFileSync(tmp, JSON.stringify({ tools: mcpTools, cwd: this.cwd, ipcPort: this.ipcPort, ipcSecret: this.ipcSecret }, null, 2), { mode: 0o600 });
  ...
  private writeAgentCfg(): void {
    ...
    const config = {
      ...
      mcpServers: { [mcpName]: { command: "node", args: [bridgePath, "--tools", this.toolsFilePath!], cwd: this.cwd, timeout: 1800000 } },
...
export function buildPromptParts(context: Context, includeHistory: boolean): { systemPrompt: string; userMessage: string } {
  return {
    systemPrompt: context.systemPrompt || "",
    userMessage: includeHistory ? buildConversationPrompt(context) : lastUserMessage(context),
  };
}
