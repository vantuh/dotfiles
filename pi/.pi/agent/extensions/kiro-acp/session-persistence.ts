...
export function persistenceKeyForSession(piSessionId: string, cwd: string): string {
	const cwdHash = hashText(cwd).slice(0, 12);
	return `${cwdHash}/${sanitizePathPart(piSessionId)}`;
}

export function loadPersistedKiroSession(key: string): PersistedKiroSession | null {
	try {
		const raw = readFileSync(sessionPath(key), "utf-8");
		const parsed = JSON.parse(raw) as PersistedKiroSession;
		if (parsed.version !== 1) return null;
		if (!parsed.kiroSessionId || typeof parsed.kiroSessionId !== "string") return null;
		if (!parsed.historyFingerprint || typeof parsed.historyFingerprint !== "string") return null;
...
export function savePersistedKiroSession(key: string, session: PersistedKiroSession): void {
	try {
...
export function historyFingerprintBeforeCurrentUser(context: Context): string {
	...
	return fingerprintMessages(lastUserIndex >= 0 ? messages.slice(0, lastUserIndex) : messages);
}
...
export function historyFingerprintAfterAssistantTurn(
	...
	return fingerprintMessages([...(context.messages || []), assistant] as Array<Context["messages"][number] | AssistantMessage>);
}
...
function fingerprintMessages(messages: Array<Context["messages"][number] | AssistantMessage>): string {
	return hashText(JSON.stringify(messages.map(normalizeMessage)));
}

function normalizeMessage(message: Context["messages"][number] | AssistantMessage): unknown {
