...
async function runSingleAgent(
	...
	if (agent.skills && agent.skills.length > 0) {
		const skillPaths = resolveSkillPaths(agent.skills, cwd ?? defaultCwd);
		args.push("--no-skills");
		for (const sp of skillPaths) args.push("--skill", sp);
	}
...
const AgentScopeSchema = StringEnum(["user", "project", "both"] as const, {
	description: 'Which agent directories to use. Default: "both" (global + project-local agents).',
	default: "both",
});
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		description: [
			...
			'Default agent scope is "both" (global ~/.pi/agent/agents + project-local .pi/agents).',
			'Set agentScope: "user" to disable project-local agents.',
		].join(" "),
