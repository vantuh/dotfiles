...
local function treesitter_try_attach(buf, language)
  ...
  if has_textobject_query then
    attach_textobject_moves(buf)
  end
...
local function attach_for_filetype(args)
  ...
  if language then
    attach_language(args.buf, language)
  end
end
...
vim.api.nvim_create_autocmd({ 'BufReadPost', 'BufNewFile' }, {
  ...
  callback = function(args)
    attach_language(args.buf, 'angular')
  end,
})
...
for _, buf in ipairs(vim.api.nvim_list_bufs()) do
  if vim.api.nvim_buf_is_loaded(buf) then
    ...
    if name:match '%.component%.html$' or name:match '%.container%.html$' then
      ...
      if filetype ~= '' then
        attach_for_filetype { buf = buf, match = filetype }
      end
    end
  end
end
