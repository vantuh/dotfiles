...
local function close_snacks_windows()
  ...
  for _, win in ipairs(wins) do
    if vim.api.nvim_win_is_valid(win) and #vim.api.nvim_list_wins() > 1 then
      ...
      if vim.startswith(ft, 'snacks_') then pcall(vim.api.nvim_win_close, win, true) end
    end
  end
end
...
local function close_empty_sidebar_placeholders()
  ...
  for _, win in ipairs(wins) do
    if vim.api.nvim_win_is_valid(win) and #vim.api.nvim_list_wins() > 1 then
      ...
      if vim.startswith(ft, 'snacks_') or (name == '' and bt == '' and ft == '' and not vim.bo[buf].modified) then pcall(vim.api.nvim_win_close, win, true) end
    end
  end
end
...
local function focus_named_buffer()
  if vim.api.nvim_buf_get_name(0) ~= '' and vim.bo.buftype == '' then return end
...
vim.api.nvim_create_autocmd('User', {
  ...
  callback = function() vim.schedule(close_empty_sidebar_placeholders) end,
})
...
persistence.save = function()
  ...
  if not ok then vim.notify('Session save skipped: ' .. tostring(err), vim.log.levels.WARN) end
end
...
vim.api.nvim_create_autocmd('VimEnter', {
  ...
  nested = true,
  callback = function()
    if vim.fn.argc() == 0 and vim.api.nvim_buf_get_name(0) == '' and vim.bo[0].buftype == '' then persistence.load() end
  end,
})
...
vim.keymap.set('n', '<leader>qd', function() persistence.stop() end, { desc = 'Do Not Save Session' })
