...
- Does not split one branch into sequential commits on the same branch (see **split-to-commits**).
- These compose: **split-to-commits** first on a branch, then **split-to-prs** if slices need separate PRs.
